...
// GET - Generate PDF for Mitra Order
export async function GET(
    ...
    try {
        ...
        y += 6
        doc.setFont("helvetica", "normal")
        doc.text(order.mitra.name, 14, y)
        doc.text(order.stokis.name, pageWidth / 2 + 10, y)

        if (order.mitra.address) {
            y += 5
            doc.setFontSize(9)
            doc.text(order.mitra.address, 14, y)
            if (order.stokis.address) {
                doc.text(order.stokis.address, pageWidth / 2 + 10, y)
            }
        }
