...
export async function GET(request: Request) {
    try {
        ...
        // Generate Excel
        const excelBuffer = generateOrdersExcel(formattedOrders, "PO Masuk Gudang")

        return new NextResponse(excelBuffer, {
            headers: {
                "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "Content-Disposition": `attachment; filename="gudang-po-${Date.now()}.xlsx"`,
            },
        })
    } catch (error) {
        console.error("Error generating Gudang PO Excel:", error)
